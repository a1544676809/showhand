 ...
 *
 * The engine is deliberately synchronous so that a hand can be replayed and
 * verified offline: given (serverSeed, clientSeed) anyone can recompute the
 * exact deck. `crypto.subtle.digest` is async, which would infect the whole
 * reducer, so we carry our own implementation. It is checked against the
 * NIST/FIPS-180-4 test vectors in `sha256.test.ts`.
 */
...
export function sha256Bytes(message: Uint8Array): Uint8Array {
  ...
  // 0x80, then zero padding, then a 64-bit big-endian bit length.
